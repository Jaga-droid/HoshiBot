...
class Music(commands.Cog, wavelink.WavelinkMixin):
    ...
    async def on_message(self,message):
        ...
        if(mention in message.content):
            embed=discord.Embed(title="**List Of Commands**",description="Yahallo everyone.!! My prefix is 'h$'. I can only play youtube songs tho.", color=discord.Color.pink())
            embed.set_thumbnail(url='https://i.pinimg.com/originals/bb/b8/7c/bbb87c5e17c8458a42376ffab837b8f2.jpg')
            embed.add_field(name="h$**play**",value="Use this command to play a song. **h$play __URL__** OR **h$play __type_your_keywords__**. **h$play** to resume a paused song",inline=False)
            embed.add_field(name="h$**pause**",value="Use this to pause a song currently playing",inline=False)
            embed.add_field(name="h$**skip**",value="Use this to skip to the next track in queue",inline=False)
            embed.add_field(name="h$**skipto**",value="Use this to play the n'th track in queue.",inline=False)
            embed.add_field(name="h$**prev**",value="Use this to play the previous track in queue",inline=False)
            embed.add_field(name="h$**queue**",value="Use this to view the list of songs currently in the queue",inline=False)
            embed.add_field(name="h$**loop**",value="Use **h$loop curr** to loop current song, **h$loop full** to loop whole queue, **h$loop nil** to remove loop",inline=False) 
            embed.add_field(name="h$**nowp**",value="Use this to check which song is playing currently", inline=False)
            embed.add_field(name="h$**seek**",value="Use this to go to a specific timestamp in the audio file. E.g.: **h$seek 2:30**", inline=False)
            embed.add_field(name="h$**stop**",value="Use this to stop a song currently playing.This command also empties the queue.", inline=False)
            embed.add_field(name="h$**lyrics**",value="LOLOLOLOLL, use this to get the lyrics of current song in the queue :rofl: :rofl:",inline=False)
            embed.add_field(name="h$**shuffle**",value="Use this to shuffle the list of songs currently in the queue",inline=False)
            embed.add_field(name="h$**volume**",value="Use **h$volume up** and **h$volume down**",inline=False)
            embed.add_field(name="h$**eq**",value="Use **n!eq 'choice'** where __choice__ can be **['flat', 'boost', 'metal', or 'piano']**",inline=False)
            embed.add_field(name="h$**aeq**",value="This is an advanced equalizer. Usage involves **n!aeq __band_number__ __gain__ .**. Band number is [1-15] or (20, 40, 63, 100, 150, 250, 400, 450, 630, 1000, 1600, 2500, 4000, 10000, 16000). Gain is [-10,10] ",inline=False)
            embed.add_field(name="h$**dc**",value="Use this to make xerneas leave the vc",inline=False)   
